...
from .utils import rescale_image, threshold, find_bbox, crop_img


def one_option_of_pro(img, cwd):
    """
    get image data make effects on ir order to increase the result of the ocr
    """
    img = rescale_image(img)
    thresh = threshold(img)
    cnts, bbox = find_bbox(thresh)
    img = crop_img(img, bbox)
    cv2.imwrite(cwd + path_dic_data + '/final_img1.png', img)
